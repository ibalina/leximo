class UsersController < ApplicationController
  ...
  def show
    #@user = current_user
	  ...
	  @words_submitted = @user.words.paginate(:all, :limit => 20, 
	                                          :order => 'words.id DESC', :per_page =>10, 
	                                          :page => params[:page])
...
#	      	:limit => 6, :order => 'votes.id DESC')	   
  ...
  def update
    @user = current_user
	  ...
	  if @user.update_attributes(:name  => params[:user][:name], :location => params[:user][:location], 
	                             :web => params[:user][:web], :bio  => params[:user][:bio])
